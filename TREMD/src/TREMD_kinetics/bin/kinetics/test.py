...
def err_log_rate(r, weight_name='sum_weight', diff_from_est=False):
    ...
    # http://stackoverflow.com/questions/26614465/python-pandas-apply-function-if-a-column-value-is-not-null
     c['estd_p'] = c.apply(lambda x: np.float64(x['rate'])*np.exp(np.sqrt(x[weight_name])) if pd.notnull(x['rate'])  else 0, axis=1)
     c['std_m'] = c.apply(lambda x: np.float64(x['rate'])*np.exp(-np.sqrt(x[weight_name])) if pd.notnull(x['rate'])  else 0, axis=1)
