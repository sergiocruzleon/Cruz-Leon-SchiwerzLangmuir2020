...
def err_log_rate(r, weight_name='sum_weight', diff_from_est=False):
    ...
    #original 
    #c['std_p'] = c.apply(lambda x: np.float64(x['rate'])*np.exp(1.0/ np.sqrt(x[weight_name])) if pd.notnull(x['rate'])  else 0, axis=1)
    #c['std_m'] = c.apply(lambda x: np.float64(x['rate'])*np.exp(-1.0/ np.sqrt(x[weight_name])) if pd.notnull(x['rate'])  else 0, axis=1)
    ...
    if diff_from_est:
      ...
      #original
      # c['err_m'] = c.apply(lambda x: np.absolute(np.float64(x['rate']) - x['std_m']) if pd.notnull(x['rate']) else 0, axis=1)
      # c['err_p'] = c.apply(lambda x: np.absolute(np.float64(x['rate']) - x['std_p']) if pd.notnull(x['rate']) else 0, axis=1)
