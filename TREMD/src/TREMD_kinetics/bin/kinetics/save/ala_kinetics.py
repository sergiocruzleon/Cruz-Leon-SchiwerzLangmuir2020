...
def phi_psi(traj, phi_res, psi_res):
    ...
    for ts in traj.trajectory:
        time_l.append(ts.time)
        phi_l.append(phi_res.dihedral())
        psi_l.append(psi_res.dihedral())

    return np.column_stack((time_l, phi_l, psi_l))


def calc_phi_psi_multi(trj_l, pdb, split_str="_demux", split_pos=0 ):
    """
    Calculate the phi psi angles for a set of trajectories.
    """
    trj_d = {}
    for trj in trj_l:
        trj_name = os.path.basename(trj)
        trj_n = trj_name.split(split_str)[split_pos]
        out_name = "{}_phi_psi.txt".format(trj_n)
        if os.path.exists(out_name):
            o = np.genfromtxt(out_name)
            print "{} already exists".format(out_name)
        else:
          u = MDAnalysis.Universe(pdb, trj)
          phi_res2 = u.residues[1].phi_selection()
          psi_res2 = u.residues[1].psi_selection()
          o = phi_psi(u, phi_res2, psi_res2)
          np.savetxt(out_name, o)
        trj_d[trj_n] = o

    return trj_d


def invert_sign_dih(dih_ar, lower_border, upper_border):
    n = deepcopy(dih_ar)
    m = np.ma.logical_and(dih_ar >= lower_border, dih_ar <= upper_border)
    n[m] = n[m]* -1
    return n
    

def process_dih_sign(trj_l, lower_upper_col1=[None, None], lower_upper_col2=[None, None]):
    """

    :param trj_l:
    :param lower_upper_col1:
    :param lower_upper_col2:
    :return:
    """

    for traj in trj_l:
        traj_fn = os.path.basename(traj)
        traj_fn = os.path.splitext(traj_fn)[0]
        p = np.genfromtxt(traj)
        if lower_upper_col1:
           p[:, 1] = invert_sign_dih(p[:, 1], lower_upper_col1[0], lower_upper_col1[1])
        if lower_upper_col2:
           p[:, 2] = invert_sign_dih(p[:, 2], lower_upper_col2[0], lower_upper_col2[1])
        np.savetxt(traj_fn+"_p.txt", p)
...
def plot_all_rama(trj_d):
    for trj_nu, phi_psi_ar in trj_d.items():
        f = plot_rama(phi_psi_ar)
        f.savefig("psi_phi_{}.png".format(trj_nu))


def multi_tp(rc_ar, state_def, verbose=False):
    """
    ...
    for ci, col in enumerate(rc_ar[0, 1:]):
        # not ideal as all trajectories could be on a transition state
    ...
    events_d = find_events_multi_rc(o, rc_ar, state_def, verbose=verbose)
    return o, events_d
...
def find_events_multi_rc(out_ar, rc_ar, state_def, verbose=False):
    """
    New strategy requires three loops instead of one.
    Probably slower.
    """
    events_end_d = find_end_points(out_ar, rc_ar)
    events_time_d = find_start_points(out_ar, rc_ar, events_end_d, state_def,
                                      verbose=verbose)
    return events_time_d
    
    
def find_end_points(out_ar, rc_ar):
    """
    This loop could be integrated in assign_multi_states, but
    seperating the functionalities might enable more flexibility.
    """ 
...
def find_multi_tp(rc_ar, state_def, out_ar, verbose=False): # Remove?
    """
    """
    ...
    events_d = defaultdict(list)

    for ri, row in enumerate(rc_ar[1:, :]):

        out_l, on_tp, events_start_time = state_row(row, prev_row, state_def, on_tp, events_start_time)
        #print on_tp
        out_ar[ri + 1, 1:] = out_l

        #if on_tp:
        #    events_start_time = row[0]

       # if verbose:
       #     print events_start_time, row

        if not np.allclose(out_l, prev_row):
            #print out_l, row
            #print prev_row
            # The states have changed and the transition is over.

            # here I need to check whether transition was instantaneous
            if not events_start_time:
                events_start_time = rc_ar[ri , 0]
               # print events_start_time
            events_d[tuple(prev_row) + tuple(out_l)].append((events_start_time, row[0]))
            on_tp = False
            events_start_time = None

        prev_row = out_l

    return out_ar, events_d
...
def multi_pop_pd(state_data_f, state_def, total_frames):
    """
...
def ala4_temp_pop(state_data_f, state_def, rep_ar, total_frames=None):
...
def count_events_at_temp(all_tp_temp_df, number_states):
...
def run_multi_tp(trj_rc_dict, rep_ar, state_def, return_raw_events=False,
    ...
    state_df
    tp_temp_df
    raw_events (optionally)

    The basics of creating an empty data frame.
    http://technicaltidbit.blogspot.de/2013/06/create-empty-dataframe-in-pandas.html
    ...
    for trj_i, rc in trj_rc_dict.items():
        ...
        trj_bin, _events = multi_tp(rc, state_def)
...
#def empty_rate_temp_table(r_df, temp_l, number_states):
#    trans_names = possible_transitions(number_states)
#    for t in temp_l:
...
def get_rates_pop_multi_temp(q_dict, replica_ar,run_name, state_def=[[-100, 50], [0, 150]], return_raw_events=False,
                             remd=True, recalc=False, time_unit_factor=1000.0, split_state_tp=False, verbose=False): #verbose not used atm 
    """
    returns:
...
def block_rates_pop_m(state_df, replica_ar, all_tp_temp_df, state_defin, number_blocks, verbose=False, time_step=1, time_unit_factor=1000.0):
    ...
    for bi, block in enumerate(blks[1:]):     
        ...
        # I think  the function will work correctly as it will take first line from rep_ar to determine a temperature list
        # the temperature won't be in a nice order- as in the full rep_ar, but each entry exists only once and the correct
        # beheviour should occur
...
def split_react_prod_tp(tp_df, tba_state, verbose=False):
    ...
    for ri, row in tp_df.iterrows() :
        # is the defintion of TP length a good one in practice?
...
def err_log_rate(r, weight_name='sum_weight', diff_from_est=False):
    ...
    c['std_p'] = c.apply(lambda x: 1.0/(np.sqrt(x[weight_name])) if pd.notnull(x['rate'])  else 0, axis=1)
    c['std_m'] = c.apply(lambda x: -1.0/(np.sqrt(x[weight_name])) if pd.notnull(x['rate'])  else 0, axis=1)
    

    # New Flage diff_from_est ? 
    if diff_from_est:
        c['err_m'] = c.apply(lambda x: (x['std_m']*x['std_m']*x['std_m']) if pd.notnull(x['rate']) else 0, axis=1)
        c['err_p'] = c.apply(lambda x: (x['std_m']*x['std_m']*x['std_m']) if pd.notnull(x['rate']) else 0, axis=1)
#        c['err_p'] = c.apply(lambda x: (x['std_m']*1.0/(np.x[weight_name])) if pd.notnull(x['rate']) else 0, axis=1)

       # np.absolute(md_r_s_ln[md_r_s_ln.type==(1,0)].rate - md_r_s_ln[md_r_s_ln.type==(1,0)].std_m
       # c['err_p'] = c.apply(lambda x: (x['rate'])*(x['std_p']) if pd.notnull(x['rate']) else 0, axis=1)
       # c['err_m'] = c.apply(lambda x: (x['rate'])*(x['std_p']) if pd.notnull(x['rate']) else 0, axis=1)

       # c['err_p'] = c.apply(lambda x: np.exp(x['std_p']) if pd.notnull(x['rate']) else 0, axis=1)
       # c['err_m'] = c.apply(lambda x: np.exp(-x['std_m']) if pd.notnull(x['rate']) else 0, axis=1)
    return c
...
def combine_stages_df(name_suffix, s, rc_d, rep_ar_d, stage_l=['md_st1', 'md_st2', 'md_st3'], n_blocks=5,
                      ...
                      dir_name='st1', return_comb_pt=False): 
    """
    ...
    Can this be generalised to CG-RNA REMD?
    """
    # What is the purpose of this function? get the combined blocks for block averaged error
    # combined the stages to get aggregated statistic
    st_st_c = []
    tp_st_c = []
    p_st_c = []
    r_st_c = []
...
def sym_counts_calc_rate(r_df, p_df, s,  trans_names, total_time, time_unit_factor=1000, verbose=False):
    """
...
def loop_reweight_events(events_st1, rep_ar, tp_rc_d, p_tp_q_all_not, tp_bc,
                        neglect_small_weights=10**-2, verbose=False):
    col = ['temperature', 'type', 'traj', 'start', 'stop', 'fraction']
...
def calc_ptpq_all_temp(raw_events_d, rc_dict, trans_type, rep_ar, tba_df,
    ...
    bins = bins=np.arange(0,1, step=0.02)
    tp_c, tp_b = np.histogram(p_q_tp[:,1], bins=bins)
    tp_bc =  get_bin_centres(tp_b)
    ...
    tp_rew, tp_prob_d = loop_reweight_events(raw_events_d, rep_ar, tp_rc_d, p_tp_q_all_not,
                              tp_bc, neglect_small_weights=False, verbose=True)
